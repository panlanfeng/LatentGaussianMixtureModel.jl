...
nF = 282
srand(100)
n_ij = round(Int64, rand(Poisson(15), nF).+rand(Exponential(95), nF));
N = sum(n_ij)
...
#LatentGaussianMixtureModel.loglikelihoodratioEM(X, Y, groupindex, Ctrue+1, ntrials=2, debuginfo=true, ctauparallel=true, ngh=100)
...
srand(b);wi_init, mu_init, sigmas_init, betas_init, ml_tmp = LatentGaussianMixtureModel.latentgmmEM(X, Y, groupindex, 1, [1., 1.], [1.0], [0.], [1.], maxiteration=200, debuginfo=false, Qmaxiteration=6)

 gamma_init = LatentGaussianMixtureModel.predictgamma(X, Y, groupindex, wi_init, mu_init, sigmas_init, betas_init);

wi_init, mu_init, sigmas_init, ml_tmp = LatentGaussianMixtureModel.gmm(gamma_init, C0, ones(C0)/C0, quantile(gamma_init, linspace(0, 1, C0+2)[2:end-1]), ones(C0), an=1/nF, maxiter=2000)

@time wi, mu, sigmas, betas, ml_C0 = LatentGaussianMixtureModel.latentgmmEM(X, Y, groupindex, C0, betas_init, wi_init, mu_init, sigmas_init, maxiteration=2000, an=an1, debuginfo=false, sn=std(gamma_init).*ones(C0), tol=.001, Qmaxiteration=6)
